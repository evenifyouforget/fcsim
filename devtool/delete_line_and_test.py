...
from typing import List, Optional  # Corrected imports for Python 3.8 compatibility
...
class MutationTester:
    ...
    def __init__(self, file_path: Path, test_cwd: Path, test_command: List[str]):
        ...
        self.edit_counter: int = (
            0  # Counter for successful deletions since last baseline re-check
        )
    ...
    def _run_test_command(
        ...
        try:
            logging.info(
                f"Running command: {' '.join(command)} in directory: {cwd if cwd else 'current'}"
            )
            ...
            if (
                ...
                or no_tests_collected_match
                or no_tests_ran_match
            ):
                is_successful = False
            ...
            logging.info(
                f"Command finished. Failures detected: {failures}, Successful Run: {is_successful}, Duration: {command_duration:.2f}s"
            )
            ...
            error_msg = (
                f"Error: Command '{command[0]}' not found. Make sure it's in your PATH."
            )
    ...
    def _get_baseline_failures(self) -> int:
        ...
        if not baseline_result.is_successful:
            logging.error(
                f"Baseline test run indicates an issue: {baseline_result.error_message}. Please fix your test setup before running the mutation tester."
            )
    ...
    def _generate_report(self):
        ...
        avg_time_per_run_final = (
            self.total_command_execution_time / self.total_command_runs
            if self.total_command_runs > 0
            else 0
        )
        ...
        logging.info(
            f"Total lines successfully deleted: {self.total_successful_deletions}"
        )
        ...
        logging.info(
            f"File '{self.file_path}' has been left with all successful deletions up to the last known good state or an improved state."
        )
    ...
    def run_mutation_process(self):
        ...
        self.last_known_good_content = list(
            self.original_content
        )  # Changed List[str](...) to list(...)
        ...
        try:
            while True:
                ...
                current_lines_at_pass_start = self.file_path.read_text().splitlines(
                    keepends=True
                )
                ...
                current_file_state_for_pass = list(
                    current_lines_at_pass_start
                )  # Changed List[str](...) to list(...)
                ...
                lines_to_attempt_deletion_in_random_order = list(
                    current_lines_at_pass_start
                )  # Changed List[str](...) to list(...)
                ...
                for line_to_attempt_delete in lines_to_attempt_deletion_in_random_order:
                    ...
                    try:
                        index_in_current_state = current_file_state_for_pass.index(
                            line_to_attempt_delete
                        )
                    except ValueError:
                        # This line was already deleted in a previous successful attempt in this pass. Skip.
                        logging.info(
                            f"Skipping already deleted line ({lines_processed_in_pass_counter}/{total_lines_in_pass})"
                        )
                        continue

                    # Create a new list of lines with the current line removed.
                    test_deletion_lines = (
                        current_file_state_for_pass[:index_in_current_state]
                    ...
                    logging.info(
                        f"Attempting to delete line ({lines_processed_in_pass_counter}/{total_lines_in_pass}) (originally: '{line_to_attempt_delete.strip()[:50]}...')"
                    )

                    current_test_run_result = self._run_test_command(
                        self.test_command, self.test_cwd
                    )

                    is_acceptable_change = (
                        current_test_run_result.is_successful
                        and current_test_run_result.failures
                        <= self.current_target_failures
                    )
                    ...
                    if (
                        ...
                        logging.info(
                            f"IMPROVEMENT DETECTED! Failures decreased from {self.current_target_failures} to {current_test_run_result.failures}."
                        )
                        logging.info(
                            "Keeping this improved version and exiting the loop."
                        )
                        ...
                        self.exit_reason = (
                            "Improvement detected (failure rate decreased)."
                        )
                        return

                    elif is_acceptable_change:
                        logging.info(
                            f"SUCCESS: Line deleted. Failures: {current_test_run_result.failures} (baseline: {self.current_target_failures})"
                        )
                        ...
                        self.last_known_good_content = list(
                            current_file_state_for_pass
                        )  # Changed List[str](...) to list(...)
                    else:
                        regression_details = (
                            current_test_run_result.error_message
                            if not current_test_run_result.is_successful
                            else f"{current_test_run_result.failures} failures (baseline: {self.current_target_failures})"
                        )
                        logging.warning(
                            f"REGRESSION: Line caused an issue: {regression_details}. Reverting."
                        )
                    ...
                    if (
                        self.edit_counter > 0
                        and self.edit_counter % RECHECK_INTERVAL == 0
                    ):
                        logging.info(
                            f"--- Performing periodic baseline re-check (every {RECHECK_INTERVAL} successful edits) ---"
                        )

                        state_before_recheck = list(
                            self.file_path.read_text().splitlines(keepends=True)
                        )  # Changed List[str](...) to list(...)
                        self.file_path.write_text("".join(self.original_content))

                        recheck_result = self._run_test_command(
                            self.test_command, self.test_cwd
                        )

                        if (
                            recheck_result.is_successful
                            and recheck_result.failures
                            <= self.initial_baseline_failures
                        ):
                            logging.info(
                                f"Periodic re-check PASSED! Original baseline ({self.initial_baseline_failures}) still met. Current failures: {recheck_result.failures}."
                            )
                            self.file_path.write_text("".join(state_before_recheck))
                            self.edit_counter = 0
                        else:
                            logging.error(
                                f"Periodic re-check FAILED! Original baseline ({self.initial_baseline_failures}) not met. Current failures: {recheck_result.failures}. Issue: {recheck_result.error_message}"
                            )
                            logging.error(
                                "This indicates a severe regression or issue with the test environment that even the original file cannot pass."
                            )
                            self._restore_file(
                                self.last_known_good_content,
                                "periodic re-check failure",
                            )
                            self.edit_counter = 0
                ...
                logging.info(
                    f"--- Pass {self.pass_count} Summary: {successful_deletions_this_pass} lines successfully deleted. Duration: {pass_duration:.2f} seconds ---"
                )
                logging.info(
                    f"--- Overall Stats: Elapsed Time: {elapsed_time:.2f}s | Total Runs: {self.total_command_runs} | Avg Time/Run: {avg_time_per_run:.2f}s ---"
                )

                if successful_deletions_this_pass == 0:
                    logging.info(
                        "No lines could be safely deleted in this pass. Breaking the loop."
                    )
                    break
...
if __name__ == "__main__":
    ...
    if len(sys.argv) < 4:
        print(
            "Usage: python script.py <file_path> <cwd_for_test_command> <test_command> [args...]"
        )
        print("\nArguments:")
        print("  <file_path>           The path to the source code file to be mutated.")
        print(
            "  <cwd_for_test_command> The current working directory where the test command will be executed."
        )
        print("                        Use '.' for the current directory.")
        print(
            "  <test_command> [args...] The exact command for your test suite (e.g., 'pytest test_my_module.py' or just 'pytest')."
        )
    ...
    if not test_cwd_arg.is_dir():
        logging.error(
            f"Error: Current working directory for tests not found or is not a directory at '{test_cwd_arg}'"
        )

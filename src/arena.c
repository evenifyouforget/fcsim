...
const char *block_fragment_shader_src = "#ifdef GL_ES\n"
...
const char *joint_vertex_shader_src = "attribute vec2 a_coords;"
                                      "void main() {"
...
void pixel_to_world(struct view *view, int x, int y, float *x_world,
                    float *y_world) {
  *x_world = view->x + view->scale * (2 * x - view->width);
  *y_world = view->y + view->scale * (2 * y - view->height);
}

void world_to_view(struct view *view, float x, float y, float *x_view,
                   float *y_view) {
...
struct joint *joint_hit_test_exclude_rod(struct arena *arena, float x, float y,
                                         struct rod *rod, bool attached) {
  struct design *design = &arena->design;
  struct joint *best_joint = NULL;
  struct joint *joint;
  double best_dist = 8.0f;
...
struct joint *joint_hit_test_exclude_wheel(struct arena *arena, float x,
                                           float y, struct wheel *wheel,
                                           bool attached) {
  struct design *design = &arena->design;
  struct joint *best_joint = NULL;
  struct joint *joint;
...
void mark_overlaps(struct arena *arena) {
  ...
  for (contact = arena->world->m_contactList; contact;
...
// Returns true if all player blocks are legal (not overlapping and inside build
// area)
...
void attach_new_rod(struct design *design, struct block *block,
                    struct joint *joint) {
...
void attach_new_wheel(struct arena *arena, struct block *block,
                      struct joint *joint) {
  struct design *design = &arena->design;
...
void detach_new_rod(struct design *design, struct block *block, double x,
                    double y) {
  struct rod *rod = &block->shape.rod;
...
void detach_new_wheel(struct arena *arena, struct block *block, double x,
                      double y) {
  struct design *design = &arena->design;
  struct wheel *wheel = &block->shape.wheel;
...
void action_new_wheel(struct arena *arena, int x, int y) {
  ...
  joint =
      joint_hit_test_exclude_wheel(arena, x_world, y_world, wheel, attached);
...
bool inside_area(struct area *area, double x, double y) {
  ...
  return x > area->x - w_half && x < area->x + w_half && y > area->y - h_half &&
         y < area->y + h_half;
}
...
void mouse_down_tool(struct arena *arena, float x, float y) {
  ...
  if (arena->tool == TOOL_WHEEL || arena->tool == TOOL_CW_WHEEL ||
...
void arena_mouse_button_down_event(struct arena *arena, int button) {
  ...
  if (arena_mouse_click_button(arena))
    return;

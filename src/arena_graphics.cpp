...
uint32_t block_graphics_layer::push_vertex(float x, float y, float r, float g,
                                           float b) {
...
uint32_t block_graphics::push_vertex(float x, float y, color col,
                                     int z_offset) {
  ensure_layer(z_offset);
  return layers[z_offset].push_vertex(x, y, col);
}

void block_graphics_layer::push_triangle(uint32_t v1, uint32_t v2,
                                         uint32_t v3) {
...
void block_graphics::push_triangle(uint32_t v1, uint32_t v2, uint32_t v3,
                                   int z_offset) {
  ensure_layer(z_offset);
...
void block_graphics::clear() { layers.clear(); }
...
double fps_tracker_t::get_tps(size_t interval) {
  ...
  return 1000 * (records_tick[index] - records_tick[prev_index]) /
         (records_ms[index] - records_ms[prev_index]);
}
...
std::vector<joint> generate_joints(block *block) {
  // The order here may not match the data model; visually it makes no difference
...
static void block_graphics_add_rect_single(struct block_graphics *graphics,
  ...
  uint32_t v1 = graphics->push_vertex(shell.x + wc - hs, shell.y + ws + hc, col,
                                      z_offset);
  uint32_t v2 = graphics->push_vertex(shell.x - wc - hs, shell.y - ws + hc, col,
                                      z_offset);
  uint32_t v3 = graphics->push_vertex(shell.x - wc + hs, shell.y - ws - hc, col,
                                      z_offset);
  uint32_t v4 = graphics->push_vertex(shell.x + wc + hs, shell.y + ws - hc, col,
                                      z_offset);
...
static void block_graphics_add_rect(struct block_graphics *graphics,
                                    struct shell shell, int type_id,
                                    int z_offset, color overlay) {
  ...
  if (type_id == FCSIM_GOAL_RECT && std::abs(shell.angle) >= CLAMP_ANGLE) {
    ...
    block_graphics_add_rect_single(
        graphics, shell_copy,
        alpha_over(get_color_by_type(FCSIM_GOAL_RECT_OVERTURNED, 0), overlay),
        z_offset + 1);
  }
  if (graphics->simple_graphics) {
    block_graphics_add_rect_single(
        graphics, shell, alpha_over(get_color_by_type(type_id, 1), overlay),
        z_offset + 1);
    return;
  }
  ...
  if (type_id == FCSIM_ROD || type_id == FCSIM_SOLID_ROD) {
    ow = w;
    oh = h;
    if (type_id == FCSIM_ROD && !graphics->wireframe)
      oh *= 2;
    iw = std::abs(ow - 4);
    ih = std::abs(oh - 4);
  } else {
  ...
  shell.rect.h = oh;
  block_graphics_add_rect_single(
      graphics, shell, alpha_over(get_color_by_type(type_id, 0), overlay),
      z_offset);
  shell.rect.w = iw;
  shell.rect.h = ih;
  block_graphics_add_rect_single(
...
void block_graphics_add_line(block_graphics *graphics, b2Vec2 start, b2Vec2 end,
                             double radius, color col, int z_offset) {
  ...
  uint32_t v3 =
      graphics->push_vertex(start.x - dx3, start.y - dy3, col, z_offset);
  uint32_t v4 =
      graphics->push_vertex(start.x - dy3, start.y + dx3, col, z_offset);
...
static void block_graphics_add_circ(struct block_graphics *graphics,
  ...
  if (graphics->simple_graphics) {
    block_graphics_add_circ_single(
        graphics, shell, alpha_over(get_color_by_type(type_id, 1), overlay),
        z_offset + 1);
    return;
  }
...
void block_graphics_add_joint(block_graphics *graphics, joint joint,
                              color overlay) {
  ...
  for (i = 0; i < circle_segments - 1; i++) {
    graphics->push_triangle(v_last - i * 2, v_last - i * 2 - 1,
                            v_last - i * 2 - 2, z_offset);
    graphics->push_triangle(v_last - i * 2 - 3, v_last - i * 2 - 1,
                            v_last - i * 2 - 2, z_offset);
  }
...
void block_graphics_push_and_bind(block_graphics *graphics) {
  ...
  glBufferData(GL_ELEMENT_ARRAY_BUFFER,
               graphics->_indices.size() * sizeof(uint32_t),
               &(graphics->_indices[0]), GL_STATIC_DRAW);

  glBindBuffer(GL_ARRAY_BUFFER, graphics->_coord_buffer);
  glBufferData(GL_ARRAY_BUFFER, graphics->_coords.size() * sizeof(float),
               &(graphics->_coords[0]), GL_DYNAMIC_DRAW);

  glBindBuffer(GL_ARRAY_BUFFER, graphics->_color_buffer);
  glBufferData(GL_ARRAY_BUFFER, graphics->_colors.size() * sizeof(float),
               &(graphics->_colors[0]), GL_STATIC_DRAW);
}
...
void block_graphics_draw(struct block_graphics *graphics, struct view *view,
                         bool use_world_transform = true) {
  block_graphics_push_and_bind(graphics);

  glUseProgram(block_program);

  if (use_world_transform) {
    glUniform2f(block_program_scale_uniform, 1.0f / (view->width * view->scale),
    ...
    glUniform2f(block_program_scale_uniform, 2.0f / view->width,
                2.0f / view->height);
...
void on_button_clicked(arena *arena, ui_button_single &button) {
  ...
  if (button.id == ui_button_id{2, 1}) {
    arena->single_ticks_remaining =
        arena->single_ticks_remaining == -1 ? 0 : -1;
  }
  ...
  if (button.id == ui_button_id{4, 11}) {
    change_speed_factor(arena, NO_CHANGE,
  ...
  if (button.id == ui_button_id{10, 0}) {
    ...
    piece_color_palette_offset =
        (piece_color_palette_offset + 1) % FCSIM_NUM_PALETTES;
  }
}
...
void regenerate_ui_buttons(arena *arena) {
  ...
  }
  {
    ...
    button.texts.push_back(
        ui_button_text{is_running(arena) ? "Stop" : "Start", 1, 0, -8});
  ...
  }
  {
    ...
    button.texts.push_back(ui_button_text{
        arena->single_ticks_remaining == -1 ? "Pause" : "Resume", 1});
  ...
  }
  {
    ...
    button.texts.push_back(
        ui_button_text{arena->autostop_on_solve ? "Cancel" : "On solve", 1});
  ...
  {
    ui_button_single button{
        {4, 0}, top_bar_x_offset + 120 + 55 * 4.5f, vh, 60 + 55 * 9 + 8, 128};
    button.enabled = arena->ui_speedbar_opened;
    all_buttons->buttons.push_back(button);
  }
  {
  ...
  {
    ui_button_single button{
        {4, 3}, top_bar_x_offset + 120 + 55, vh - 30, 50, 50, 2};
    button.enabled = arena->ui_speedbar_opened;
  ...
  {
    ui_button_single button{
        {4, 4}, top_bar_x_offset + 120 + 55 * 2, vh - 30, 50, 50, 2};
    button.enabled = arena->ui_speedbar_opened;
  ...
  {
    ui_button_single button{
        {4, 6}, top_bar_x_offset + 120 + 55 * 4, vh - 30, 50, 50, 2};
    button.enabled = arena->ui_speedbar_opened;
  ...
  {
    ui_button_single button{
        {4, 7}, top_bar_x_offset + 120 + 55 * 5, vh - 30, 50, 50, 2};
    button.enabled = arena->ui_speedbar_opened;
  ...
  {
    ui_button_single button{
        {4, 8}, top_bar_x_offset + 120 + 55 * 6, vh - 30, 50, 50, 2};
    button.enabled = arena->ui_speedbar_opened;
  ...
  {
    ui_button_single button{
        {4, 9}, top_bar_x_offset + 120 + 55 * 7, vh - 30, 50, 50, 2};
    button.enabled = arena->ui_speedbar_opened;
  ...
  {
    ui_button_single button{
        {4, 10}, top_bar_x_offset + 120 + 55 * 8, vh - 30, 50, 50, 2};
    button.enabled = arena->ui_speedbar_opened;
  ...
  {
    ui_button_single button{
        {4, 11}, top_bar_x_offset + 120 + 55 * 9, vh - 30, 50, 50, 2};
    button.enabled = arena->ui_speedbar_opened;
...
float draw_text_default(arena *arena, std::string text, float x, float y,
                        float scale = FONT_SCALE_DEFAULT) {
  text_set_scale(scale);
  text_stream_update(&arena->tick_counter, text.c_str());
  text_stream_render(&arena->tick_counter, arena->view.width,
                     arena->view.height, (int)x, (int)y);
...
void draw_tick_counter(struct arena *arena) {
  ...
  const bool is_running_arena_and_not_paused =
      is_running_arena && arena->single_ticks_remaining == -1;
  ...
  size_t tps_interval =
      std::min<size_t>(fps_tracker_t::buffer_size - 1,
                       std::max<size_t>(1, (size_t)(fps_value * 2)));
  ...
  if (arena->fine_adjustment_factor != 0) {
    x += FONT_X_INCREMENT * FONT_SCALE_DEFAULT * 1;
    x = draw_text_default(arena, std::to_string(arena->fine_adjustment_factor),
                          x, 10);
  ...
  if (actual_checksum != 0) {
    if (expect_checksum != 0) {
      x = draw_text_default(
          arena, actual_checksum == expect_checksum ? "[OK]" : "[!]", x, y);
    }
...
void draw_ui(arena *arena) {
  ...
  for (auto it = all_buttons->buttons.begin(); it != all_buttons->buttons.end();
       ++it) {
    if (!it->enabled)
      continue;
    shell ishell{SHELL_RECT, {}, it->x, it->y, 0};
    ishell.rect = {it->w, it->h};
    block_graphics_add_rect(graphics, ishell, FCSIM_UI_BUTTON, it->z_offset,
                            it->highlighted ? color{0.25, 1, 1, 1}
                                            : color{0, 0, 0, 0});
  }
...
void preview_trail_draw(arena *arena) {
  ...
  for (size_t trail_index = 0; trail_index < all_trails->trails.size();
       ++trail_index) {
    ...
    for (size_t datapoint_index = 1;
         datapoint_index < the_trail.datapoints.size(); ++datapoint_index) {
      b2Vec2 current = the_trail.datapoints[datapoint_index];
      block_graphics_add_line(graphics, last, current, LINE_RADIUS,
                              get_color_by_type(FCSIM_GOAL_CIRCLE, 0), 4);
      last = current;
    }
  }
}
...
extern "C" bool arena_mouse_click_button(struct arena *arena) {
  ...
  for (int z_offset = max_z_offset; z_offset >= 0; --z_offset) {
    for (auto it = all_buttons->buttons.begin();
         it != all_buttons->buttons.end(); ++it) {
      if (it->z_offset != z_offset)
        continue;
      float cx = arena->cursor_x;
      float cy = arena->view.height - arena->cursor_y;
      if (it->enabled && it->clickable && it->x - it->w * 0.5 <= cx &&
          cx <= it->x + it->w * 0.5 && it->y - it->h * 0.5 <= cy &&
          cy <= it->y + it->h * 0.5) {

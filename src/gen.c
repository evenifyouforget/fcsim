...
static void gen_joint(b2World *world, b2Body *b1, b2Body *b2, double x,
                      double y, int spin) {
...
static void gen_joint_stack(b2World *world, struct joint *joint) {
  ...
  if (joint->gen) {
    gen_joint(world, joint->gen->body, node->block->body, x, y,
              block_spin(node->block));
  } else if (node->next) {
    gen_joint(world, node->block->body, node->next->block->body, x, y,
              block_spin(node->next->block) - block_spin(node->block));
    node = node->next;
  }

  for (; node->next; node = node->next) {
    gen_joint(world, node->block->body, node->next->block->body, x, y,
              block_spin(node->next->block));
  }
}

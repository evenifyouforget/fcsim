...
static void make_font_tex(void) {
  ...
  data = static_cast<uint8_t *>(malloc(128 * 128));

  memset(data, 0, 128 * 128);

  for (y = 0; y < 8; y++) {
    for (x = 0; x < 16; x++)
      make_font_tex_letter(data + x * 8 + y * 1024, 128,
                           &font[(x + y * 16) * 8]);
  }

  glGenTextures(1, &font_tex);
  glBindTexture(GL_TEXTURE_2D, font_tex);
  glTexImage2D(GL_TEXTURE_2D, 0, GL_ALPHA, 128, 128, 0, GL_ALPHA,
...
void text_set_scale(float scale) { _text_scale = scale; }
...
void text_render(struct text *text, int w, int h, int x, int y) {
  ...
  glUniform2f(text_program_shift_uniform, (float)(x * 2 - w) / w,
              (float)(y * 2 - h) / h);
...
void text_stream_update(struct text_stream *text, const char *str) {
  ...
  data = static_cast<float *>(calloc(len * 24, sizeof(float)));

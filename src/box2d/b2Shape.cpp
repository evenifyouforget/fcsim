...
static void PolyMass(b2MassData *massData, const b2Vec2 *vs, int32 count,
                     float64 rho) {
  b2Vec2 center;
  b2Vec2_Set(&center, 0.0, 0.0);
  float64 area = 0.0;
  float64 I = 0.0;
  ...
  for (int32 i = 0; i < count; ++i) {
    ...
    float64 intx2 = inv3 * (0.25 * (ex1 * ex1 + ex2 * ex1 + ex2 * ex2) +
                            (px * ex1 + px * ex2)) +
                    0.5 * px * px;
    float64 inty2 = inv3 * (0.25 * (ey1 * ey1 + ey2 * ey1 + ey2 * ey2) +
                            (py * ey1 + py * ey2)) +
                    0.5 * py * py;
...
void b2ShapeDef_ComputeMass(const b2ShapeDef *shapeDef, b2MassData *massData) {
  ...
  case e_circleShape: {
    ...
    massData->mass =
        shapeDef->density * b2_pi * circle->radius * circle->radius;
    b2Vec2_Set(&massData->center, 0.0, 0.0);
...
static void b2CircleShape_ctor(b2CircleShape *circleShape,
                               const b2ShapeDef *def, b2Body *body,
                               const b2Vec2 &localCenter);

extern "C" {
void b2PolyShape_ctor(b2PolyShape *polyShape, const b2ShapeDef *def,
                      b2Body *body, const b2Vec2 newOrigin);
};

b2Shape *b2Shape_Create(const b2ShapeDef *def, b2Body *body, b2Vec2 center) {
  switch (def->type) {
  case e_circleShape: {
    b2CircleShape *mem = (b2CircleShape *)b2BlockAllocator_Allocate(
        &body->m_world->m_blockAllocator, sizeof(b2CircleShape));
  ...
  case e_boxShape:
  case e_polyShape: {
    b2PolyShape *mem = (b2PolyShape *)b2BlockAllocator_Allocate(
        &body->m_world->m_blockAllocator, sizeof(b2PolyShape));
...
static void b2Shape_dtor(b2Shape *shape) {
  if (shape->m_proxyId != b2_nullProxy) {
    b2BroadPhase_DestroyProxy(shape->m_body->m_world->m_broadPhase,
                              shape->m_proxyId);
  }
}
...
static void b2CircleShape_ctor(b2CircleShape *circleShape,
  ...
  b2Vec2 r =
      b2Mul(circleShape->m_shape.m_body->m_R, circleShape->m_localPosition);
  circleShape->m_shape.m_position = circleShape->m_shape.m_body->m_position + r;

  b2AABB aabb;
  b2Vec2_Set(&aabb.minVertex,
             circleShape->m_shape.m_position.x - circleShape->m_radius,
             circleShape->m_shape.m_position.y - circleShape->m_radius);
  b2Vec2_Set(&aabb.maxVertex,
             circleShape->m_shape.m_position.x + circleShape->m_radius,
             circleShape->m_shape.m_position.y + circleShape->m_radius);

  b2BroadPhase *broadPhase = circleShape->m_shape.m_body->m_world->m_broadPhase;
  if (b2BroadPhase_InRange(broadPhase, aabb)) {
    circleShape->m_shape.m_proxyId =
        b2BroadPhase_CreateProxy(broadPhase, aabb, circleShape);
  } else {
...
void b2CircleShape_Synchronize(b2Shape *shape, b2Vec2 position1,
                               const b2Mat22 *R1, b2Vec2 position2,
                               const b2Mat22 *R2) {
  b2CircleShape *circleShape = (b2CircleShape *)shape;
  shape->m_R = *R2;
  shape->m_position =
      position2 + b2Mul(shape->m_R, circleShape->m_localPosition);
  ...
  b2AABB aabb;
  b2Vec2_Set(&aabb.minVertex, lower.x - circleShape->m_radius,
             lower.y - circleShape->m_radius);
  b2Vec2_Set(&aabb.maxVertex, upper.x + circleShape->m_radius,
             upper.y + circleShape->m_radius);
...
void b2CircleShape_ResetProxy(b2Shape *shape, b2BroadPhase *broadPhase) {
  ...
  b2AABB aabb;
  b2Vec2_Set(&aabb.minVertex, shape->m_position.x - circleShape->m_radius,
             shape->m_position.y - circleShape->m_radius);
  b2Vec2_Set(&aabb.maxVertex, shape->m_position.x + circleShape->m_radius,
             shape->m_position.y + circleShape->m_radius);
...
void b2PolyShape_ctor(b2PolyShape *polyShape, const b2ShapeDef *def,
  ...
  if (def->type == e_boxShape) {
    ...
    polyShape->m_localCentroid =
        def->localPosition + b2Mul(localR, centroid) - newOrigin;
    for (int32 i = 0; i < polyShape->m_vertexCount; ++i) {
      ...
      polyShape->m_coreVertices[i] =
          polyShape->m_vertices[i] - 2.0 * b2_linearSlop * u;
    }
  }
  ...
  polyShape->m_shape.m_position =
      polyShape->m_shape.m_body->m_position +
      b2Mul(polyShape->m_shape.m_body->m_R, polyShape->m_localCentroid);
  ...
  b2Vec2 position = polyShape->m_shape.m_position +
                    b2Mul(polyShape->m_shape.m_R, polyShape->m_localOBB.center);
  b2AABB aabb;
  ...
  if (b2BroadPhase_InRange(broadPhase, aabb)) {
    polyShape->m_shape.m_proxyId =
        b2BroadPhase_CreateProxy(broadPhase, aabb, polyShape);
  } else {
...
void b2PolyShape_Synchronize(b2Shape *shape, b2Vec2 position1,
                             const b2Mat22 *_R1, b2Vec2 position2,
                             const b2Mat22 *_R2) {
...

  {
    ...
    b2Vec2 h = b2Mul(absR, polyShape->m_localOBB.extents);
    b2Vec2 center = position1 + b2Mul(R1, polyShape->m_localCentroid +
                                              polyShape->m_localOBB.center);
...

  {
    ...
    b2Mat22 absR = b2Abs(obbR);
    b2Vec2 h = b2Mul(absR, polyShape->m_localOBB.extents);
    b2Vec2 center = position2 + b2Mul(R2, polyShape->m_localCentroid +
                                              polyShape->m_localOBB.center);
...
b2Vec2 b2PolyShape_Support(const b2Shape *shape, b2Vec2 d) {
  ...
  return shape->m_position +
         b2Mul(shape->m_R, polyShape->m_coreVertices[bestIndex]);
}
...
bool b2PolyShape_TestPoint(b2Shape *shape, b2Vec2 p) {
  ...
  for (int32 i = 0; i < polyShape->m_vertexCount; ++i) {
    float64 dot =
        b2Dot(polyShape->m_normals[i], pLocal - polyShape->m_vertices[i]);
    if (dot > 0.0) {
      return false;
    }
  }
...
void b2PolyShape_ResetProxy(b2Shape *shape, b2BroadPhase *broadPhase) {
  ...
  b2Vec2 h = b2Mul(absR, polyShape->m_localOBB.extents);
  b2Vec2 position =
      shape->m_position + b2Mul(shape->m_R, polyShape->m_localOBB.center);
  b2AABB aabb;
  aabb.minVertex = position - h;
  aabb.maxVertex = position + h;

...
void b2ContactSolver_ctor(b2ContactSolver *solver, b2Contact **contacts,
                          int32 contactCount, b2StackAllocator *allocator) {
  ...
  solver->m_constraints = (b2ContactConstraint *)b2StackAllocator_Allocate(
      solver->m_allocator,
      solver->m_constraintCount * sizeof(b2ContactConstraint));
    ...
    for (int32 j = 0; j < c->pointCount; ++j) {
      {
        ...
        b2Vec2 dv = b2->m_linearVelocity + b2Cross(b2->m_angularVelocity, r2) -
      ...
      // Solver tangent constraints
      {
        ...
        float64 newImpulse =
            b2Clamp(ccp->tangentImpulse + lambda, -maxFriction, maxFriction);
...
bool b2ContactSolver_SolvePositionConstraints(b2ContactSolver *solver,
  ...
  for (int32 i = 0; i < solver->m_constraintCount; ++i) {
    ...
    for (int32 j = 0; j < c->pointCount; ++j) {
      ...
      float64 C = beta * b2Clamp(separation + b2_linearSlop,

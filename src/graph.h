...
struct joint {
  ...
  bool in_drag_set;       /* true while this joint is in the affected set of an
                             ...
                             State */
  uint64_t _checksum_uid; /* scratch value used during design checksum computation;
...
struct area {
  double x, y;
  double w, h;
  double expand; /* outward tolerance (mm) applied symmetrically to both dimensions
...
struct design {
  ...
  int expect_checksum; /* checksum from the ?checksum= URL parameter; 0 if absent.
                          User flow: export produces a share link containing this value;
                          opening that link re-loads the design and compares against
                          actual_checksum to verify the save/load cycle was lossless. */
  int actual_checksum; /* design checksum computed by recalculate_design_checksum();
                          0 until first recalculation. Shown as base-36 in the debug
                          overlay alongside [OK] (matches expect) or [!] (mismatch). */
};

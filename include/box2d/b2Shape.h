...
struct b2Shape {
  ...
  void (*Synchronize)(b2Shape *shape, b2Vec2 position1, const b2Mat22 *R1,
...
// position of the centroid. The vertices of the incoming polygon are
// pre-rotated according to the local rotation. The vertices are also shifted to
// be centered on the centroid. Since the local rotation is absorbed into the
// vertex coordinates, the polygon rotation is equal to the body rotation.
// However, the polygon position is centered on the polygon centroid. This
// simplifies some collision algorithms.
...
b2Shape *b2Shape_Create(const b2ShapeDef *def, b2Body *body, b2Vec2 newOrigin);

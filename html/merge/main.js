...
let gl_env = {
  ...
  glGetUniformLocation(program, name) {
    return add_object(
      gl.getUniformLocation(get_object(program), make_cstring(name)),
    );
  },
...
if (import_checksum_str) {
  import_checksum = parseInt(
    import_checksum_str.split("").reverse().join(""),
    36,
  );
...
function on_text(text) {
  console.log(text);
  let export_checksum_str = last_exported_checksum
    .toString(36)
    .split("")
    .reverse()
    .join("");
  design_link.innerHTML = design_link.href =
    self_url_full() + "?designId=" + text + "&checksum=" + export_checksum_str;
...
if (!design_id && !level_id) {
  ...
  if (notification) {
    ...
    helpDiv.appendChild(
      chainElement(
        ["p"],
        "To load levels or designs, use ?levelId or ?designId",
      ),
    );
    helpDiv.appendChild(chainElement(["p", "b"], "Examples"));
    helpDiv.appendChild(
      chainElement(["p"], linkElement(self_url_full() + "?levelId=646726")),
    );
    helpDiv.appendChild(
      chainElement(["p"], linkElement(self_url_full() + "?designId=12483401")),
    );
    helpDiv.appendChild(
      chainElement(
        ["p"],
        "To merge, add something with the word `DesignId` in it. You can merge 3 or more at a time!",
      ),
    );
    helpDiv.appendChild(
      chainElement(["p"], linkElement(self_url_full() + "?designId=12718206&mergeDesignId=12718207")),
    );
